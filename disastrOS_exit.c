...
void internal_exit(){
  ...
  // since he called  wait or waitpid, we wake him up
    ...
    //we release all semaphors of a process upon termination
    while(running->sem_descriptors.first) {
      SemDescriptor* sem_des=(SemDescriptor*) running->sem_descriptors.first;
      ...
      SemDescriptorPtr_free(sem_des->ptr);
